...
def read_results(data_dir, cutoff=300, runs_per_inst=100, suffix="train"):
    fl_name = "{}/validate-random-{}/validationRunResultLineMatrix-cli-1-" \
              "walltimeworker.csv".format(data_dir, suffix)

    if not os.path.exists(fl_name):
        raise ValueError("{} does not exist".format(fl_name))
    ...
    data = list()
    sat_data = list()
    print('total_data: {}'.format(len(tmp_data)))
...
def get_data(scenario, data_dir, sc_dict, retrieve=["SAT", "UNSAT"], log=False):
    data_dir = data_dir + "/" + sc_dict[scenario]["scen"] + "/"
    runtimes, inst_ls, sat_ls = \
        read_results(data_dir=data_dir, cutoff=sc_dict[scenario]["cutoff"], runs_per_inst=100, suffix="train")
    print("Train data loaded")
    try:
        test_runtimes, test_inst_ls, test_sat_ls = \
            read_results(data_dir=data_dir, cutoff=sc_dict[scenario]["cutoff"], runs_per_inst=100, suffix="test")
    ...
    if "UNSAT" not in retrieve:
        ...
        runtimes, features, sat_ls = preprocess.remove_instances_with_status(
            runningtimes=runtimes, features=features, sat_ls=sat_ls,
            status="UNSAT", log=log)

    features = preprocess.feature_imputation(features, impute_val=-512, impute_with="median")
